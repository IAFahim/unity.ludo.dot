...
public class FacebookManager : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        // 🟢 FIX: Auto login if session exists after initialization
        if (FB.IsInitialized && FB.IsLoggedIn && Facebook.Unity.AccessToken.CurrentAccessToken != null)
        {
            Debug.Log("🟢 FIX: Auto login with existing Facebook session");
...

    {
...

        {
            ...
            // 🟢 FIX: Restore session automatically after initialization
            if (Facebook.Unity.AccessToken.CurrentAccessToken != null)
            {
                Debug.Log("🟢 FIX: Auto session restore from SetInit()");
                PlayerPrefs.SetString("LoggedType", "Facebook");
                initSession();
            }
    ...
    private void initSession()
    {
        ...
        if (logType.Equals("Facebook"))
        {
            if (Facebook.Unity.AccessToken.CurrentAccessToken == null)
            {
                Debug.Log("🟡 FIX: No access token found yet, delaying initSession...");
                StartCoroutine(WaitForFacebookToken());
                return;
            }

            GameManager.Instance.facebookIDMy = Facebook.Unity.AccessToken.CurrentAccessToken.UserId;
            StartCoroutine(DelayFBInit()); // 🟢 FIX: Added small delay for stability
            LoggedIn = true;
    ...
    // 🟢 FIX: Added delay to allow FB SDK token setup
    IEnumerator WaitForFacebookToken()
    {
        float t = 0;
        while (Facebook.Unity.AccessToken.CurrentAccessToken == null && t < 5f)
        {
        ...
        if (Facebook.Unity.AccessToken.CurrentAccessToken != null)
        {
            Debug.Log("🟢 FIX: Token found, resuming session");
            initSession();
        }
        else
        {
            Debug.Log("🔴 FIX: Token not found after waiting.");
        }
    }

    // 🟢 FIX: Delay API calls to prevent null FB.API responses
    IEnumerator DelayFBInit()
    {
